...
int main() {
    ...
    const char* primaryGraphPath = "src/data/full_dataset/graphswith100nodes/ug3.100";
    const char* fallbackGraphPath = "src/data/full_dataset/graphswith100nodes/ug3.100";
    const char* buildFallbackGraphPath = "../../src/data/full_dataset/graphswith100nodes/ug3.100";
    ...
    if (loaded) {
        ...
        SpatialGrid detectionGrid(
            minX,
            maxX,
            minY,
            maxY,
            static_cast<int>(myGraph.nodes.size()),
            static_cast<int>(myGraph.edges.size())
        );

        // --- STEP 3: CONSTRUCT THE FINAL PLANAR GRAPH ---
        // Now the PlanarizedGraph has everything it needs to build the "Chicken"

        myGraph.applyInitialForceDirectedLayout(50); // Optional: Pre-layout the graph for better initial visualization
